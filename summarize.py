...
def make_context(
    ...
    if chat_format == "chatml":
        ...
        [tokenizer.im_start_id,tokenizer.im_end_id] = tokenizer.convert_tokens_to_ids([im_start, im_end])
...
def main(args):
    ...
    if args.eval_task == 'code_completion':
        ...
        dataset_name = "/Qwen2-7B/cnn_dailymail/cnn_dailymail.py"
    ...
    metric_tensorrt_llm = [evaluate.load("/Qwen2-7B/evaluate-0.4.1/metrics/rouge") for _ in range(num_beams)]
    metric_hf = [evaluate.load("/Qwen2-7B/evaluate-0.4.1/metrics/rouge") for _ in range(num_beams)]
