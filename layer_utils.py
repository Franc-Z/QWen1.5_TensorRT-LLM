...
def build_qkv(
    ...
    if len(qkv_modules) == 1:
        ...
        for m in qkv_modules:
            ...
            #assert not (hasattr(m, "bias") and m.bias is not None) #把此处注释掉，为了处理带BIAS的情况
        ...
        q_bias = qkv_modules[0].bias.detach()   # I add it to deal with bias
        k_bias = qkv_modules[1].bias.detach()   # I add it to deal with bias
        v_bias = qkv_modules[2].bias.detach()   # I add it to deal with bias
    ...
    #config.q.bias_scaling_factor = q_bias_scaling_factor
    #config.q.bias_scaling_factor_2 = q_bias_scaling_factor_2
